        ...
        <View style={styles.container}>
            ...
            <ScrollView
                ...
                onMomentumScrollEnd={e => {
                  ...
                  const { locations.latitude, locations.longitude } = this.state.locations[place];

                  this.mapView.animateToCoordinate({locations.latitude, locations.longitude}, 1000);
                  setTimeout(() => { mark.showCallout(); }, 1000);
                }}
            >
            {
              this.state.locations.map(location => (
                  <View key={locations.key} style={styles.cardContainer} source={locations.image}>
                      <View style={styles.subContainer}>
                          <Text style={styles.title}>{locations.title}</Text>
                          <Text style={styles.description}>{locations.description}</Text>
                      </View>
                  </View>
                )
              )
