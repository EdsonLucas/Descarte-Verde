...
export default class Pontos extends Component {
    ...
    _renderItem ({item, index}) {
        return (
            <ImageBackground key={item.key} style={styles.cardContainer} source={{ uri: item.image }}>
                <LinearGradient start={{x: 0, y: 0.25}} end={{x: 0, y: 1}} colors={['transparent', '#ffffff','#ffffff']} style={styles.subContainer}>
    ...
    render () {
        return (
            <LinearGradient start={{x: 0, y: 0.05}} end={{x: 0, y: 1}} colors={['#00c398', '#6ed39b','#cceeb0']} style={styles.container}>
